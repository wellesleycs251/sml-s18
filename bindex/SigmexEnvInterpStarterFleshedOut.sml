...
structure SigmexEnvInterpStarter = struct
  ...
  and eval (Int i) env = i 
    ...
    | eval (Sigma(name, lo, hi, body)) env =
      let val loVal = eval lo env
	  val hiVal = eval hi env
	  val nameRange = Utils.range loVal (hiVal + 1)
	  val valRange = map (fn nameVal =>
				 eval body (Env.bind name nameVal env))
			     nameRange
      in List.foldl op+ 0 valRange
      end
