...
structure SigmexStarter = struct
  ...
  ************************************************************)
  ...
  and expToSexp (Int i) = Sexp.Int i
    ...
    | expToSexp (Sigma(name, lo, hi, body)) =
      Seq [Sym "sigma", Sym name, expToSexp lo, expToSexp hi,
	   expToSexp body]
    (* add a Sigma clause here *)	   
  ...
  and freeVarsExp (Int i) = S.empty
    ...
    | freeVarsExp (Sigma(name,lo,hi,body)) =
      S.union (S.difference (freeVarsExp body) (S.singleton name))
	      (S.union (freeVarsExp lo) (freeVarsExp hi))
